...
void lmm_PXEM(const arma::vec& y, //应变量
                      const arma::mat& X, //协变量, 第一个为1定义为截距项 == cbind(1, 协变量), 没有协变量时需要输入1的一个向量
                      const arma::mat& G, //SNP矩阵
                      const int& maxIter,
                      double& sigma2e,
                      double& sigma2b,
                      arma::vec& betax,
...
//' @param y  gene expression vector.
//' @param X  covariates file for eQTL data.                  协变量矩阵
//' @param G  normalized genotype (cis-SNPs) matrix for eQTL.    SNP矩阵
//' @param maxIter  maximum iteration (default is 1000).
...
SEXP lmm_PXEM(const arma::vec y, const arma::mat X, const arma::mat G, const bool PXEM, const int maxIter){
    ...
    vec betax =zeros<vec>(X.n_cols); // 协变量的效应
    int iter;
	mat covb = zeros<mat>(X.n_cols, X.n_cols);// 协变量的效应个数的一个方阵
    vec mub  = zeros<vec>(G.n_cols);// SNP效应估计

    lmm_PXEM(y, X, G, maxIter, sigma2e, sigma2b, betax, loglik, iter, covb, mub, PXEM);
